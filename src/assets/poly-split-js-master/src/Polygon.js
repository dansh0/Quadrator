...
export default class Polygon {
  ...
  indDistance = (point) => {
    ...
    p = line.getSegmentNearestPoint(point);
    l = (p.subtraction(point)).length();
    if (l < distance)
      distance = l;
    return distance;
  };
  ...
  splitNearestEdge = (point) => {
    ...
    line = new Line(this.poly.get(this.poly.size() - 1), this.poly.get(0));
    p = line.getSegmentNearestPoint(point);
    l = p.subtraction(point).length();
    if (l < distance) {
      distance = l;
  ...
  static findCutLine = (square, res, cutLine) => {
    ...
    if (!res.leftTriangle.empty() && square < res.leftTriangleSquare) {
      ...
      p = res.trapezoid.get(0).addition(res.trapezoid.get(3).subtraction(res.trapezoid.get(0)).multiplication(m));
      ...
      p = res.rightTriangle.get(2).addition(res.rightTriangle.get(1).subtraction(res.rightTriangle.get(2)).multiplication(m));
      if (res.p3_exist) {
  ...
  static getCut = (l1, l2, s, poly1, poly2, cut) => {
    ...
    if (sn1 > 0) {
      ...
      findCutLineRes = Polygon.findCutLine(sn2, res, cut)
      cut = findCutLineRes.cutLine;
      if (findCutLineRes.value) {
        cut = cut.reverse();
        return {
  ...
  static isPointInsidePoly = (poly, point) => {
    ...
    line = new Line(poly.get(pointsCount), poly.get(0));
    res = l.crossSegmentSegment(line);
    result += res.value;
    return result % 2 != 0;
  };
